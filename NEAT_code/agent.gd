...
var body = null#load(Params.agent_body_path).instance()
...
func _init(neural_net: NeuralNet, is_leader_clone: bool, bd: Node2D) -> void:
	"""Called by genome.generate_agent(), requires a neural network to work.
	"""
	network = neural_net
	
	# Get and reset the agent
	body = bd
	if not body:
		print("Body is null")
	body.resetAgent()
