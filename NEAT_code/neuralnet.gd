...
func save_to_json(name: String) -> void:
	...
	if dir.open("user://network_configs/" + name.split('/')[0]) == ERR_INVALID_PARAMETER:
		dir.make_dir("user://network_configs/" + name.split('/')[0])
	if dir.open("user://network_configs/" + name.split('/')[0] + '/' + name.split('/')[1]) == ERR_INVALID_PARAMETER:
		dir.make_dir("user://network_configs/" + name.split('/')[0] + '/' + name.split('/')[1])
